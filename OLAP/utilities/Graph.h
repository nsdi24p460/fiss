...
public:
    ...
    /**
         * Print graph in circle if exist
         */
    void search_circle()
    {
        ...
        if (!_edges.empty())
        {
            ...
            {
                visited.insert(std::make_pair(iter->first, false));
            }
    ...
    void dfs(V v, std::map<V, bool> &visited, std::vector<V> &circle, std::ofstream &outfile)
    {
        visited[v] = true;
        circle.push_back(v);
        for (auto next : _edges[v].children)
        {
            if (std::find(circle.begin(), circle.end(), next) == circle.end())
            {
                if (!visited[next])
                {
                    dfs(next, visited, circle, outfile);
                }
            }
            else
            {
